...
export default function QuizAttempt() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#F4F4F9] p-4 relative overflow-hidden">
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-red-100 text-center max-w-md relative z-10">
          <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <AlertTriangle className="w-10 h-10 text-red-500" />
          </div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Unable to Start Quiz</h2>
  ...
  if (result) {
    ...
    return (
      <div className="min-h-screen bg-[#F4F4F9] flex items-center justify-center p-4 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
          <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-purple-200/30 blur-[100px]" />
          <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-blue-200/30 blur-[100px]" />
        </div>

        <div className="bg-white rounded-[2.5rem] shadow-sm max-w-4xl w-full overflow-hidden relative z-10 grid grid-cols-1 lg:grid-cols-2">
          <div className={`p-10 flex flex-col items-center justify-center text-center ${isPassed ? 'bg-gradient-to-br from-[#C9C7F5]/20 to-[#A9C1F7]/20' : 'bg-red-50/50'}`}>
            <div className={`w-32 h-32 rounded-full flex items-center justify-center mb-6 shadow-sm border-4 border-white ${isPassed ? 'bg-[#C9C7F5] text-[#5a59b5]' : 'bg-red-100 text-red-500'}`}>
              {isPassed ? <Trophy className="w-16 h-16" /> : <XCircle className="w-16 h-16" />}
            </div>

            <h2 className="text-3xl font-bold text-gray-900 mb-2">{isPassed ? 'Congratulations!' : 'Keep Practicing'}</h2>
            <p className="text-gray-500 mb-8 max-w-xs">{isPassed ? 'You have successfully passed the quiz.' : 'Don\'t worry, you can try again to improve.'}</p>

            <div className="flex items-center gap-4 mb-8">
              <div className="text-center px-6 py-4 bg-white rounded-2xl shadow-sm">
                <div className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Score</div>
                <div className="text-3xl font-bold text-gray-900">{result.score}<span className="text-lg text-gray-400">/{result.totalMarks}</span></div>
              </div>
              <div className="text-center px-6 py-4 bg-white rounded-2xl shadow-sm">
                <div className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Avg Score</div>
                <div className="text-2xl font-bold text-gray-900">{result.averageScore}</div>
              </div>
            </div>
          ...
          <div className="p-8 bg-white h-[600px] overflow-y-auto">
            ...
            <div className="space-y-4">
              {quiz.questions.map((q, index) => {
                ...
                return (
                  <div key={q.id} className="p-5 rounded-[1.5rem] border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-sm transition-all group">
                    <div className="flex items-start gap-3">
                      <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mt-0.5 ${isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                      ...
                      <div className="flex-1">
                        <p className="font-medium text-gray-900 mb-3 text-sm">
                          <span className="text-gray-400 mr-2 font-mono">My Q{index + 1}.</span>
                          {q.questionText}
                        </p>

                        <div className="text-xs space-y-2 bg-white p-3 rounded-xl border border-gray-100">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-gray-400 min-w-[60px]">You:</span>
                          ...
                          {!isCorrect && (
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-gray-400 min-w-[60px]">Correct:</span>
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F9] relative overflow-hidden flex flex-col">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-purple-200/30 blur-[100px]" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-blue-200/30 blur-[100px]" />
      </div>
      ...
      <header className="relative z-10 pt-6 px-6 pb-2">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between bg-white/60 backdrop-blur-md p-4 rounded-[1.5rem] border border-white/50 shadow-sm">
            <div className="flex items-center gap-4">
              <button onClick={() => navigate('/dashboard')} className="p-2 hover:bg-white rounded-full transition-colors text-gray-500">
                <ArrowLeft className="w-5 h-5" />
              </button>
              <div>
                <h1 className="font-bold text-gray-900 text-lg leading-tight">{quiz.title}</h1>
                <p className="text-xs font-medium text-gray-500">{quiz.session.title}</p>
              </div>
            </div>
            ...
            {timeLeft !== null && (
              <div className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-mono font-bold transition-colors ${timeLeft < 60
                ...
                : 'bg-white text-[#5a59b5] border border-[#C9C7F5]'
                }`}>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto p-6 relative z-10 flex flex-col h-full">
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-[#C9C7F5]/20 flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="flex-1 p-8 md:p-12 flex flex-col relative">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              <span className="text-sm font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-lg">Q{currentQuestionIndex + 1}</span>
                <span>/ {quiz.questions.length}</span>
              </span>
              <div className="flex items-center gap-2 text-[#b59a5a] bg-[#F7D483]/10 px-3 py-1.5 rounded-lg border border-[#F7D483]/20">
                ...
                {JSON.parse(question.options).map((option, index) => (
                  <button
                    ...
                    className={`group w-full text-left p-5 rounded-[1.25rem] border-2 transition-all duration-200 relative overflow-hidden ${answers[question.id] === option
                      ? 'border-[#C9C7F5] bg-[#C9C7F5]/10 text-[#5a59b5] shadow-sm'
                      : 'border-gray-100 hover:border-[#C9C7F5]/50 hover:bg-gray-50 text-gray-600'
                      }`}
                  >
                    <div className="flex items-center gap-5 relative z-10">
                      <div className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center flex-shrink-0 transition-colors ${answers[question.id] === option
                        ? 'border-[#C9C7F5] bg-white text-[#5a59b5]'
                        : 'border-gray-200 text-gray-300 group-hover:border-[#C9C7F5]/50'
                        }`}>
          ...
          <div className="p-6 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between relative z-10">
            <button
